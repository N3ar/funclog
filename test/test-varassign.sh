...
# Run LLVM VarAssign
echo "[*] **** RUNNING VARASSIGN PASS"
if ! opt -load-pass-plugin="${BUILD}/lib/libVarAssign.so" -passes="varassign" -S "instr-${NAME}.ll" -o "vars-${NAME}.ll" ; then
    echo "[-] opt failed to run varassign"
    do_exit 1
fi

# Compile Instrumented LLVM
echo "[*] **** Building instrumented executable"
if ! clang -llogger "vars-${NAME}.ll" -o "${NAME}" -v ; then
